...
if ((isset($_POST['name'])) && (isset($_POST['mail'])) && isset($_POST['password']) && (isset($_POST['numero']))) {
  if(!filter_var($_POST['mail'],FILTER_VALIDATE_EMAIL)){
    $err = "email-sign-in";
    header("Location:addnewUser.php?salt=".$err);
    die();
  }
  if(strlen($_POST['numero']) < 9 || strlen($_POST['numero']) > 9){
    $err = "num-sign-in";
    header("Location:addnewUser.php?nalt=".$err);
    die();
  }
  if (strlen($_POST['numero']) == 9) {
    $pattern = '/[a-z]|[A-Z]/i';
    $pattern2 = '/^32|^33|^34/i';
    $res = preg_match($pattern,$_POST['numero']);
    $res2 = preg_match($pattern2,$_POST['numero']);
    if(($res == 1) || ($res2 == 0)){
      $err = "num-sign-in";
      header("Location:addnewUser.php?nalt=".$err);
      die();
  ...
  include('code_include/inscrit.php');
  // cree une session
...
<body>
  ...
  <script src="dist/js/dist/util.js"></script>
  <script src="dist/js/dist/popover.js"></script>
